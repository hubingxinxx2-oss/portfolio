...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    const primeFont = async () => {
      const { text: label, fontFamily, fontWeight } = propsRef.current;
      if (!label || !document.fonts?.load || !fontFamily) return;
      const firstFamily = String(fontFamily)
        .split(',')[0]
        .trim()
        .replace(/^['"]|['"]$/g, '');
      const fam = /\s/.test(firstFamily) ? `"${firstFamily}"` : firstFamily;
      try {
        await document.fonts.load(`${fontWeight || 400} 96px ${fam}`, label);
        await document.fonts.ready;
      } catch {
        // 字体加载失败时回退到默认字体，避免阻塞渲染
      }
    };
